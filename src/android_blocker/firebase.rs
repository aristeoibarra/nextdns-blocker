...
impl FirebaseClient {
    ...
    /// Write the dashboard_state node to Firebase RTDB.
    /// Contains stats, categories, and pending actions for the Android dashboard.
    pub fn set_dashboard_state(&self, state: &serde_json::Value) -> Result<(), AppError> {
        let url = format!(
            "{}/devices/{}/dashboard_state.json?auth={}",
            self.rtdb_url, self.device_id, self.access_token
        );
        self.agent
            .put(&url)
            .send_json(state)
            .map_err(|e| AppError::Api {
                message: format!("Firebase RTDB PUT dashboard_state failed: {e}"),
                status_code: extract_status(&e),
                hint: Some("Check Firebase config and network connectivity".to_string()),
            })?;
        Ok(())
